...
class OpenStackStyleValidator:
    ...
    def _check_line_length(self, lines):
        ...
        for i, line in enumerate(lines, 1):
            if len(line) > 79:
                self.errors.append(f"Line {i}: exceeds 79 characters ({len(line)})")
    ...
    def _check_mock_usage(self, call_node):
        ...
        if (isinstance(call_node.func, ast.Attribute) and
            ...
            call_node.func.value.id == 'mock' and
            call_node.func.attr == 'patch'):
...
def main():
    ...
    for file_path in args.files:
        ...
        if is_valid and not results['warnings']:
            if not args.quiet:
